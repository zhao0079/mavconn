/** THIS IS AN AUTOMATICALLY GENERATED FILE.  DO NOT MODIFY
 * BY HAND!!
 *
 * Generated by lcm-gen
 **/

#include <string.h>
#include "obstacle_map_message_t.h"

static int __obstacle_map_message_t_hash_computed;
static int64_t __obstacle_map_message_t_hash;
 
int64_t __obstacle_map_message_t_hash_recursive(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for (fp = p; fp != NULL; fp = fp->parent)
        if (fp->v == __obstacle_map_message_t_get_hash)
            return 0;
 
    const __lcm_hash_ptr cp = { p, (void*)__obstacle_map_message_t_get_hash };
    (void) cp;
 
    int64_t hash = 0xd7e792ab42044963LL
         + __int8_t_hash_recursive(&cp)
         + __float_hash_recursive(&cp)
         + __int32_t_hash_recursive(&cp)
         + __int32_t_hash_recursive(&cp)
         + __int32_t_hash_recursive(&cp)
         + __int32_t_hash_recursive(&cp)
         + __int32_t_hash_recursive(&cp)
         + __int32_t_hash_recursive(&cp)
         + __int64_t_hash_recursive(&cp)
         + __int8_t_hash_recursive(&cp)
        ;
 
    return (hash<<1) + ((hash>>63)&1);
}
 
int64_t __obstacle_map_message_t_get_hash(void)
{
    if (!__obstacle_map_message_t_hash_computed) {
        __obstacle_map_message_t_hash = __obstacle_map_message_t_hash_recursive(NULL);
        __obstacle_map_message_t_hash_computed = 1;
    }
 
    return __obstacle_map_message_t_hash;
}
 
int __obstacle_map_message_t_encode_array(void *buf, int offset, int maxlen, const obstacle_map_message_t *p, int elements)
{
    int pos = 0, thislen, element;
 
    for (element = 0; element < elements; element++) {
 
        thislen = __int8_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].type), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __float_encode_array(buf, offset + pos, maxlen - pos, &(p[element].resolution), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].num_rows), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].num_cols), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].map_r0), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].map_c0), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].array_r0), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].array_c0), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].length), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int8_t_encode_array(buf, offset + pos, maxlen - pos, p[element].data, p[element].length);
        if (thislen < 0) return thislen; else pos += thislen;
 
    }
    return pos;
}
 
int obstacle_map_message_t_encode(void *buf, int offset, int maxlen, const obstacle_map_message_t *p)
{
    int pos = 0, thislen;
    int64_t hash = __obstacle_map_message_t_get_hash();
 
    thislen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;
 
    thislen = __obstacle_map_message_t_encode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
 
    return pos;
}
 
int __obstacle_map_message_t_encoded_array_size(const obstacle_map_message_t *p, int elements)
{
    int size = 0, element;
    for (element = 0; element < elements; element++) {
 
        size += __int8_t_encoded_array_size(&(p[element].type), 1);
 
        size += __float_encoded_array_size(&(p[element].resolution), 1);
 
        size += __int32_t_encoded_array_size(&(p[element].num_rows), 1);
 
        size += __int32_t_encoded_array_size(&(p[element].num_cols), 1);
 
        size += __int32_t_encoded_array_size(&(p[element].map_r0), 1);
 
        size += __int32_t_encoded_array_size(&(p[element].map_c0), 1);
 
        size += __int32_t_encoded_array_size(&(p[element].array_r0), 1);
 
        size += __int32_t_encoded_array_size(&(p[element].array_c0), 1);
 
        size += __int64_t_encoded_array_size(&(p[element].length), 1);
 
        size += __int8_t_encoded_array_size(p[element].data, p[element].length);
 
    }
    return size;
}
 
int obstacle_map_message_t_encoded_size(const obstacle_map_message_t *p)
{
    return 8 + __obstacle_map_message_t_encoded_array_size(p, 1);
}
 
int __obstacle_map_message_t_decode_array(const void *buf, int offset, int maxlen, obstacle_map_message_t *p, int elements)
{
    int pos = 0, thislen, element;
 
    for (element = 0; element < elements; element++) {
 
        thislen = __int8_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].type), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __float_decode_array(buf, offset + pos, maxlen - pos, &(p[element].resolution), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].num_rows), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].num_cols), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].map_r0), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].map_c0), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].array_r0), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].array_c0), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        thislen = __int64_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].length), 1);
        if (thislen < 0) return thislen; else pos += thislen;
 
        p[element].data = (int8_t*) lcm_malloc(sizeof(int8_t) * p[element].length);
        thislen = __int8_t_decode_array(buf, offset + pos, maxlen - pos, p[element].data, p[element].length);
        if (thislen < 0) return thislen; else pos += thislen;
 
    }
    return pos;
}
 
int __obstacle_map_message_t_decode_array_cleanup(obstacle_map_message_t *p, int elements)
{
    int element;
    for (element = 0; element < elements; element++) {
 
        __int8_t_decode_array_cleanup(&(p[element].type), 1);
 
        __float_decode_array_cleanup(&(p[element].resolution), 1);
 
        __int32_t_decode_array_cleanup(&(p[element].num_rows), 1);
 
        __int32_t_decode_array_cleanup(&(p[element].num_cols), 1);
 
        __int32_t_decode_array_cleanup(&(p[element].map_r0), 1);
 
        __int32_t_decode_array_cleanup(&(p[element].map_c0), 1);
 
        __int32_t_decode_array_cleanup(&(p[element].array_r0), 1);
 
        __int32_t_decode_array_cleanup(&(p[element].array_c0), 1);
 
        __int64_t_decode_array_cleanup(&(p[element].length), 1);
 
        __int8_t_decode_array_cleanup(p[element].data, p[element].length);
        if (p[element].data) free(p[element].data);
 
    }
    return 0;
}
 
int obstacle_map_message_t_decode(const void *buf, int offset, int maxlen, obstacle_map_message_t *p)
{
    int pos = 0, thislen;
    int64_t hash = __obstacle_map_message_t_get_hash();
 
    int64_t this_hash;
    thislen = __int64_t_decode_array(buf, offset + pos, maxlen - pos, &this_hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;
    if (this_hash != hash) return -1;
 
    thislen = __obstacle_map_message_t_decode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
 
    return pos;
}
 
int obstacle_map_message_t_decode_cleanup(obstacle_map_message_t *p)
{
    return __obstacle_map_message_t_decode_array_cleanup(p, 1);
}
 
int __obstacle_map_message_t_clone_array(const obstacle_map_message_t *p, obstacle_map_message_t *q, int elements)
{
    int element;
    for (element = 0; element < elements; element++) {
 
        __int8_t_clone_array(&(p[element].type), &(q[element].type), 1);
 
        __float_clone_array(&(p[element].resolution), &(q[element].resolution), 1);
 
        __int32_t_clone_array(&(p[element].num_rows), &(q[element].num_rows), 1);
 
        __int32_t_clone_array(&(p[element].num_cols), &(q[element].num_cols), 1);
 
        __int32_t_clone_array(&(p[element].map_r0), &(q[element].map_r0), 1);
 
        __int32_t_clone_array(&(p[element].map_c0), &(q[element].map_c0), 1);
 
        __int32_t_clone_array(&(p[element].array_r0), &(q[element].array_r0), 1);
 
        __int32_t_clone_array(&(p[element].array_c0), &(q[element].array_c0), 1);
 
        __int64_t_clone_array(&(p[element].length), &(q[element].length), 1);
 
        q[element].data = (int8_t*) lcm_malloc(sizeof(int8_t) * q[element].length);
        __int8_t_clone_array(p[element].data, q[element].data, p[element].length);
 
    }
    return 0;
}
 
obstacle_map_message_t *obstacle_map_message_t_copy(const obstacle_map_message_t *p)
{
    obstacle_map_message_t *q = (obstacle_map_message_t*) malloc(sizeof(obstacle_map_message_t));
    __obstacle_map_message_t_clone_array(p, q, 1);
    return q;
}
 
void obstacle_map_message_t_destroy(obstacle_map_message_t *p)
{
    __obstacle_map_message_t_decode_array_cleanup(p, 1);
    free(p);
}
 
int obstacle_map_message_t_publish(lcm_t *lc, const char *channel, const obstacle_map_message_t *p)
{
      int max_data_size = obstacle_map_message_t_encoded_size (p);
      uint8_t *buf = (uint8_t*) malloc (max_data_size);
      if (!buf) return -1;
      int data_size = obstacle_map_message_t_encode (buf, 0, max_data_size, p);
      if (data_size < 0) {
          free (buf);
          return data_size;
      }
      int status = lcm_publish (lc, channel, buf, data_size);
      free (buf);
      return status;
}

struct _obstacle_map_message_t_subscription_t {
    obstacle_map_message_t_handler_t user_handler;
    void *userdata;
    lcm_subscription_t *lc_h;
};
static
void obstacle_map_message_t_handler_stub (const lcm_recv_buf_t *rbuf, 
                            const char *channel, void *userdata)
{
    int status;
    obstacle_map_message_t p;
    memset(&p, 0, sizeof(obstacle_map_message_t));
    status = obstacle_map_message_t_decode (rbuf->data, 0, rbuf->data_size, &p);
    if (status < 0) {
        fprintf (stderr, "error %d decoding obstacle_map_message_t!!!\n", status);
        return;
    }

    obstacle_map_message_t_subscription_t *h = (obstacle_map_message_t_subscription_t*) userdata;
    h->user_handler (rbuf, channel, &p, h->userdata);

    obstacle_map_message_t_decode_cleanup (&p);
}

obstacle_map_message_t_subscription_t* obstacle_map_message_t_subscribe (lcm_t *lcm, 
                    const char *channel, 
                    obstacle_map_message_t_handler_t f, void *userdata)
{
    obstacle_map_message_t_subscription_t *n = (obstacle_map_message_t_subscription_t*)
                       malloc(sizeof(obstacle_map_message_t_subscription_t));
    n->user_handler = f;
    n->userdata = userdata;
    n->lc_h = lcm_subscribe (lcm, channel, 
                                 obstacle_map_message_t_handler_stub, n);
    if (n->lc_h == NULL) {
        fprintf (stderr,"couldn't reg obstacle_map_message_t LCM handler!\n");
        free (n);
        return NULL;
    }
    return n;
}

int obstacle_map_message_t_unsubscribe(lcm_t *lcm, obstacle_map_message_t_subscription_t* hid)
{
    int status = lcm_unsubscribe (lcm, hid->lc_h);
    if (0 != status) {
        fprintf(stderr, 
           "couldn't unsubscribe obstacle_map_message_t_handler %p!\n", hid);
        return -1;
    }
    free (hid);
    return 0;
}

