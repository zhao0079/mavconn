...
namespace MAVCONN
{
    RegisterInputOperation(CameraInput)
        .setDescription("Captures frames from a connected camera")
        .addName("camera")
        .addName("c")
        .addParameter("index", &CameraInput::setIndex, "Internal index number of the camera", 0)
    ...
    CameraInput::CameraInput(Camera* camera) : InputOperation(camera)
    {
        ...
        this->getCamera()->setInputModeName("Camera");
