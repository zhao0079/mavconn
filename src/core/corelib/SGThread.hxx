...
protected:
    ...
    /**
     * If using deferred thread cancellation (the default),
     * calling this function cancells the thread. If using
     * SOFT cancel mode, this returns true, telling the thread
     * to do its own exit foo.
     */
    bool is_cancelled();
...
private:
    ...
    /**
     * If using SOFT_CANCEL mode, this flag signifies
     * the thread should clean up so it can join
     */
    bool cancel_flag;
    cancel_t cancel_mode;
...
inline
SGThread::SGThread() : cancel_flag(false), cancel_mode(CANCEL_DEFERRED)
{
}
...
inline bool
SGThread::is_cancelled()
{
    if (cancel_mode == CANCEL_SOFT)
        return cancel_flag;
    else {
        pthread_testcancel();
        // if the thread has been cancelled, this code will never be 
        // called anyway
        return false;
    }
}
