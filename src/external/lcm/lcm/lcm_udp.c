...
//===============================================================================
// This code adds an udpm handler that skips always to the newest message in the queue
//===============================================================================
static int
lcm_udpm_handle_nq (lcm_udpm_t *lcm)
{
    int status;
    char ch;
    ...
    /* If there are still packets in the queue, remove the current packet without handling it. */
    if (!is_buf_queue_empty (lcm->inbufs_filled))
    {
    	/*free the buffer*/
    	if (lcmb->buf_from_ringbuf)
...
static lcm_provider_vtable_t udpm_nq_vtable = {
    .create     = lcm_udpm_create,
    .destroy    = lcm_udpm_destroy,
    .subscribe  = lcm_udpm_subscribe,
    .publish    = lcm_udpm_publish,
    .handle     = lcm_udpm_handle_nq,
    .get_fileno = lcm_udpm_get_fileno,
};

static lcm_provider_info_t udpm_nq_info = {
    .name = "udpm_nq",
    .vtable = &udpm_nq_vtable,
};

void
lcm_udpm_nq_provider_init (GPtrArray * providers)
{
    g_ptr_array_add (providers, &udpm_nq_info);
}
//===============================================================================
